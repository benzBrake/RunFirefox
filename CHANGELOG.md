...
- 新增 Opera 浏览器下载支持：从官方 FTP 解析 stable/beta/dev 渠道最新版本并下载 Windows x64 离线安装包自动解压，`opera.exe` 可被自动识别为 Opera，复制系统配置支持 Opera Software 的渠道配置目录。
- 设置窗口"辅助"页的"浏览器退出后自动关闭"复选框与 Bosskey 组的"隐藏到系统托盘"复选框支持按文案宽度自动换行为两行，Bosskey 说明文字按实测宽度自适应行数，下方内容与分组框高度联动下移/加高，字母语言不再被截断或互相重叠。
- 设置窗口"Chrome++"页的"配置文件"、"当前版本"、"最新版本"与"额外匹配标题"标签支持按文案宽度自动换行为两行，两条说明文字按实测宽度自适应行数并精简了英文等译文、收紧页尾间距，字母语言不再与输入框、版本号重叠或被截断。
- 设置窗口“高级”页的插件目录、缓存位置、缓存大小标签支持按文案宽度自动换行为两行，“自动控制缓存大小”复选框过长时也自动换行，CDP 启用提示的说明文字按实测宽度自适应行数（最多可占三行），Chromium 设置分组框同步加高并相应下移命令行参数区域，字母语言不再被截断。
- 设置窗口“常规”页改为自适应布局：按当前语言文案的实测宽度流式排布控件、放不下自动换行，字母语言（英语、西班牙语等）不再出现标签重叠与版本号被遮挡；“浏览器”单独一行并加宽下拉，“立即下载”按钮右对齐于位数行行尾，最新/当前版本各占半行，长文案复选框自动换行为两行，语言与运行选项分组新增“RunFirefox 设置”标题；Waterfox 本地版本号直接显示短提示（获取失败），移除原独立提示行。
- 仓库新增 `LangCustom.ini` 自定义语言模板（含完整西班牙语 `[es-ES]` 示例），复制到 exe 同目录即可覆盖内置文案或新增语言；README 同步增加“自定义语言”使用说明。
