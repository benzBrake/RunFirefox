...
Chrome++ 标签页保留可选安装和标签操作设置。Brave、Whale 使用内置自编译 DLL，按浏览器 EXE 的实际 x86/x64 架构离线安装，与启动器位数无关；不支持 ARM64。页面显示已安装版本和内置版本，文件不一致时可手动替换为内置版，无需查询上游更新。首次安装使用 `libs/chrome_plus/chrome++.ini` 中的内置完整配置，并保留已有的用户自定义配置。
