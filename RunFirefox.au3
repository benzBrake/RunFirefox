...
;~ 	GUICtrlSetOnEvent(-1, "DownloadFirefox")
	...
	GUICtrlCreateGroup(_t("CacheSettings", "缓存设置"), 10, 75, 480, 120)
	GUICtrlCreateLabel(_t("PluginsDirectory", "插件目录"), 20, 103, 120, 20)
	$hCustomPluginsDir = GUICtrlCreateEdit($CustomPluginsDir, 140, 98, 270, 20, $ES_AUTOHSCROLL)
	GUICtrlSetTip(-1, _t("PluginsDirectoryTooltip", "浏览器插件目录\n空白=默认位置"))
	$hGetPluginsDir = GUICtrlCreateButton(_t("Browse", "浏览"), 420, 98, 60, 22)
	GUICtrlSetTip(-1, _t("SpecifyPluginsDirectoryTooltip", "选择浏览器插件目录"))
	GUICtrlSetOnEvent(-1, "GetPluginsDir")

	GUICtrlCreateLabel(_t("CacheDirectory", "缓存位置"), 20, 133, 120, 20)
	$hCustomCacheDir = GUICtrlCreateEdit($CustomCacheDir, 140, 128, 270, 20, $ES_AUTOHSCROLL)
	GUICtrlSetTip(-1, _t("CacheDirectoryTooltip", "浏览器缓存位置\n空白=默认位置"))
	$hGetCacheDir = GUICtrlCreateButton(_t("Browse", "浏览"), 420, 128, 60, 22)
	GUICtrlSetTip(-1, _t("SpecifyCacheDirectoryTooltip", "选择浏览器缓存文件夹"))
	GUICtrlSetOnEvent(-1, "GetCacheDir")

	GUICtrlCreateLabel(_t("CacheSize", "缓存大小"), 20, 163, 120, 20)
	$hCacheSize = GUICtrlCreateEdit($CacheSize, 140, 158, 60, 20, BitOR($ES_NUMBER, $ES_AUTOHSCROLL))
	GUICtrlSetTip(-1, _t("CacheSizeTooltip", "缓存大小\n空白=默认大小"))
	GUICtrlCreateLabel("MB", 215, 163, 35, 20)
	$hCacheSizeSmart = GUICtrlCreateCheckbox(_t("CacheSizeControl", " 自动控制缓存大小"), 250, 158, -1, 20)
