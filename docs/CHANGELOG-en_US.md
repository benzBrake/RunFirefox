...
- Added Opera download support: resolves the latest stable/beta/dev version from Opera's official FTP, downloads the Windows x64 offline installer and extracts it automatically; opera.exe is now auto-detected as Opera and profile import recognizes the Opera Software channel data directories.
- On the Utilities tab of the settings window, the "auto close after browser exit" checkbox and the Bosskey group's "hide to system tray" checkbox now wrap onto two lines when the current language's text is too wide, the Bosskey description sizes its height from the measured text width, and the rows below plus the group box shift down accordingly, so alphabetic languages are no longer clipped or overlapping.
- The "Config file", "Current version", "Latest version", and "Extra matched titles" labels on the Chrome++ tab of the settings window now wrap onto two lines when the current language's text is too wide, both help texts size their heights from the measured text width, their translations were shortened, and the bottom of the tab was tightened up, so alphabetic languages no longer overlap the input fields or version values or get clipped.
